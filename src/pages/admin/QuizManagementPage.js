...
export const QuizManagementPage = () => {
    ...
    const columns = [
        ...
        },
        {
            key: 'id',
            header: 'Actions',
            render: (_, row) => (_jsxs("div", { className: "flex gap-2", children: [_jsx(Button, { size: "sm", onClick: () => navigate(`/quizzes/${row.id}/edit`), children: "Edit" }), row.status === 'draft' && (_jsx(Button, { size: "sm", variant: "success", onClick: () => handlePublish(row.id), children: "Publish" })), _jsx(Button, { size: "sm", variant: "danger", onClick: () => handleDelete(row.id), children: "Delete" })] })),
        },
    ];
    const totalPages = Math.ceil(total / 10);
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-3xl font-bold text-gray-900", children: "Quiz Management" }), _jsx("p", { className: "text-gray-600 mt-2", children: "Create, edit, and manage your quizzes" })] }), _jsx(Button, { variant: "primary", size: "lg", onClick: () => navigate('/quizzes/create'), children: "+ Create Quiz" })] }), error && _jsx(Alert, { type: "error", title: "Error", onClose: () => { }, children: error }), quizzes.length === 0 ? (_jsx(EmptyState, { title: "No Quizzes Yet", description: "Create your first quiz to get started", action: _jsx(Button, { onClick: () => navigate('/quizzes/create'), children: "Create Quiz" }) })) : (_jsxs(Card, { children: [_jsx(Table, { columns: columns, data: quizzes, loading: loading }), totalPages > 1 && (_jsx(Pagination, { page: page, totalPages: totalPages, onPageChange: setPage }))] }))] }));
};
